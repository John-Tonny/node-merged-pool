...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    */
    ...
    // john
    for (var i = 0; i < rpcData.fund.length; i++) {
        var payeeReward = 0;

        payeeReward = rpcData.fund[i].amount;

        var payeeScript;

        if (rpcData.fund[i].script) {
            payeeScript = new Buffer(rpcData.fund[i].script, 'hex');
        } else {
            payeeScript = util.addressToScript(rpcData.fund[i].payee);
        }
	
        if (rpcData.fund[i].script == '51') {
            rewardToPool = payeeReward;
	    // john
	    /*
            for (var j = 0; j < recipients.length; j++){
                var recipientReward = Math.floor(recipients[i].percent * payeeReward);
                rewardToPool -= recipientReward;
    ...
    if(rpcData.height%120 == 0){
      console.log("###############  rpc data ###########");
      console.log(rpcData);
      console.log(Buffer.concat(txOutputBuffers).toString('hex'));	     
    }
